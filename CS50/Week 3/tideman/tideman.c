...
void sort_pairs(void)
{
    for (int i = 0; i < pair_count - 1; i++)
    {
        ...
        {
            int margin_i = preferences[pairs[j].winner][pairs[j].loser] - preferences[pairs[j].loser][pairs[j].winner];
            int margin_next =
                preferences[pairs[j + 1].winner][pairs[j + 1].loser] - preferences[pairs[j + 1].loser][pairs[j + 1].winner];

            if (margin_i < margin_next)
            {
                // Swap pairs
                pair temp = pairs[j];
                pairs[j] = pairs[j + 1];
...
// Check if adding a pair creates a cycle in the locked graph
bool creates_cycle(int winner, int loser)
{
    if (winner == loser)
    {
