    ...
    <body class="text-gray-400 bg-gradient-to-br from-gray-900 to-gray-800 min-h-screen">
        <div class="max-w-md mx-auto px-4 py-12">
            ...
            <div class="flex flex-col justify-center items-center space-y-6 bg-gray-800/50 p-8 rounded-2xl shadow-xl backdrop-blur-sm border border-gray-700">
                ...
                <form method="post" action="{{ route('ticket.update', $ticket->id) }}" class="w-full space-y-4">
                    @csrf
                    @method('PUT')
            
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Title</label>
                        <input type="text" name="title" required placeholder="Title" value="{{ $ticket->title }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" required placeholder="Description"
                            class="w-full px-4 py-2 rounded-xl bg-gray-900 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500 placeholder-gray-500 transition">{{ $ticket->description }}</textarea>
                    </div>
